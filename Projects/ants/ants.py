...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while distance < self.min_range and place.entrance != beehive:
            place, distance = place.entrance, distance + 1

        if distance < self.min_range:
            return None

        while not place.bees and place.entrance != beehive:
            place, distance = place.entrance, distance + 1

        return rANTdom_else_none(place.bees) if distance <= self.max_range else None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        place = self.place
        for bee in list(place.bees):  # make a copy
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting:
            self.digesting -= 1
        elif self.eat_bee(rANTdom_else_none(self.place.bees)):
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.queen_already:
            self.real = False
        else:
            QueenAnt.queen_already = True
            self.real = True
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        while p:
            place_ants = []
            if p.ant:
                place_ants.append(p.ant)
            if isinstance(p.ant, ContainerAnt) and p.ant.contained_ant:
                place_ants.append(p.ant.contained_ant)
            for ant in place_ants:
                if not ant in self.doubled_ants:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if hasattr(self, 'backwards') and self.backwards:
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        return max(2 - 0.2 * distance - 0.05 * self.insects_shot, 0)
        # END Problem OPTIONAL
