...
def repeated(t, k):
    ...
    def helper(t, times, prev):
        if k == times:
            return prev
        current = next(t)
        if current == prev:
            return helper(t, times + 1, prev)
        return helper(t, 1, current)

    return helper(t, 1, prev)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Actually it is similar to linked list
    ...
    def joint_withdraw(amount, passwd):
        ...
        return withdraw(amount, passwd)  # Maybe old or wrong
