...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            response = 'You must add ${0} more funds.'.format(
                self.price - self.balance)
        else:
            if self.balance == self.price:
                response = 'Here is your {0}.'.format(self.name)
            else:
                response = 'Here is your candy and ${0} change.'.format(
...
def is_bst(t):
    ...
    if len(t.branches) == 2:
        ...
        min_of_right = bst_min(t.branches[1])
        return max_of_left <= t.label and min_of_right > t.label
...
def deep_map(f, link):
    ...
    while link != Link.empty:
        if type(link.first) == Link:
            ele_list.append(deep_map(f, link.first))
        else:
            ele_list.append(f(link.first))
        link = link.rest

    ele_list.reverse()
    new_link = Link(ele_list[0])
    for ele in ele_list[1:]:
        new_link = Link(ele, new_link)
    return new_link
