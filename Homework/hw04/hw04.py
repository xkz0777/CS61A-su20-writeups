...
def multiple_references_explanation():
    return """
        see par1 and par2 when calculating:
        >>> inst = interval(1, 5)
        >>> par1(inst, inst)
        [0.1, 12.5]
        >>> par2(inst, inst)
        [0.5, 2.5]
        par1 evaluates R1 + R2 and R1R2 differently,
        while you cant make R1 = 1 in R1 + R2
        and make R1 = 5 in R1R2 at the same time"""
...
def quadratic(x, a, b, c):
    ...
    faint = -b/(2*a)
    if lower_bound(x) < faint and faint < upper_bound(x):
        lower = min(lower, f(faint))
        upper = max(upper, f(faint))
    return interval(lower, upper)
