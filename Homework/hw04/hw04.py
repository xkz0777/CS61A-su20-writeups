...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) < 0 or lower_bound(y) > 0, 'can not divide an interval that spans zero'
    reciprocal_y = interval(1 / upper_bound(y), 1 / lower_bound(y))
    return mul_interval(x, reciprocal_y)


def multiple_references_explanation():
    return """Par1 calculate the summation and product of the two intervals. However, 
    when the sum of two resistors is given, the range of the product is **no longer that large**.
    Therefore, the result interval given by par1 is relatively larger comparinng to par2, which uses the
    formular written in the form that no variable that represents an uncertain number is repeated.
    Thus, par2 is a better program."""
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) <= -b / (2 * a) <= upper_bound(x):
        return interval(min(m, n, extreme), max(m, n, extreme))
...
def check_par():
    ...
    True
    """
    r1 = interval(1, 7)  # Replace this line!
