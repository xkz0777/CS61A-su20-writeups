...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        def add_attack_and_defense(card):
            card.attack += other_card.attack
            card.defense += other_card.defense
            return card  # Notice that we need to return here, otherwise player.deck.cards will be None

        player.deck.cards = list(map(add_attack_and_defense, player.deck.cards))
        opponent.deck.cards = list(filter(filter_func, opponent.deck.cards))
