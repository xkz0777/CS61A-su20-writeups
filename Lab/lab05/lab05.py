...
def add_trees(t1, t2):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Zip the corresponding part and add them together using list comprehension, then add the addtional(slicing)
    len1, len2 = len(t1), len(t2)
    addtional = t1[len2:] if len1 > len2 else t2[len1:]
    return tree(label(t1) + label(t2), [add_trees(branch1, branch2) for branch1, branch2 in zip(branches(t1), branches(t2))] + addtional)
