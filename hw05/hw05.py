...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, query_password):
        if len(incorrect_list) == 3:
            return 'Too many incorrect attempts. Attempts: ' + str(incorrect_list)
        if query_password != password:
            incorrect_list.append(query_password)
...
def repeated(t, k):
    ...
    for _ in range(k-1):
        lst.append(next(t))
    for i in t:
        if all([True if item == i else False for item in lst[-k+1:]]):
            return i
        lst.append(i)
...
def remainders_generator(m):
    ...
    yield from map(lambda x: remainder(x), range(m))
