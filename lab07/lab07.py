...
def subseqs(s):
    ...
    if not s:
        ...
        nxt = subseqs(s[1:])
        return nxt + insert_into_all(s[0], nxt)
...
def make_glookup(class_assignments):
    ...
    def lookup(assignment, grade):
        person_grade[assignment] = grade
        return sum([person_grade[i] for i in person_grade]) / sum([class_assignments[i] for i in person_grade])
    return lookup
