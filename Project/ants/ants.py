...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            # The container will be stored in the place's ant instance attribute
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower a subclass of ThrowerAnt that is more costly and watersafe."""
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.has_queen:
            self.is_queen = QueenAnt.has_queen = True
        else:
            self.is_queen = False
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.is_queen:
            ...
            while place is not None:
                if place.ant is not None and place.ant not in QueenAnt.buffed_ants:
                    place.ant.damage *= 2
                    QueenAnt.buffed_ants.add(place.ant)
                if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant is not None:
                    # Note that this should not be inside the previous if suite
                    if place.ant.contained_ant not in QueenAnt.buffed_ants:
                        place.ant.contained_ant.damage *= 2
                        QueenAnt.buffed_ants.add(place.ant.contained_ant)
                place = place.exit
        else:
...
class Bee(Insect):
    ...
    def __init__(self, armor, place=None):
        ...
        self.direction = 1  # 1 means moving forward, -1 backwards
        self.scared = False
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if self.direction == -1:
            if isinstance(self.place, Hive):  # Should not move
